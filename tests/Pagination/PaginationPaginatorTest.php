...
class PaginationPaginatorTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $p = new Paginator($factory = m::mock('Illuminate\Pagination\Factory'), ['foo', 'bar', 'baz'], 3, 2);
    ...
    {
        $p = new Paginator($factory = m::mock('Illuminate\Pagination\Factory'), [], 0, 2);
        $factory->shouldReceive('getCurrentPage')->once()->andReturn(1);
        $p->setupPaginationContext();
    ...
    {
        $p = new Paginator($env = m::mock('Illuminate\Pagination\Factory'), ['foo', 'bar', 'baz'], 3, 2);
    ...
    public function testPaginatorIsIterable()
    {
        ...
        $this->assertEquals(['foo', 'bar', 'baz'], $p->getIterator()->getArrayCopy());
    }
    ...
    {
        $p = new Paginator(m::mock('Illuminate\Pagination\Factory'), ['a', 'b', 'c'], 3, 2);
        $last = $p->last();
