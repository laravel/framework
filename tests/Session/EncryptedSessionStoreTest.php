...
class EncryptedSessionStoreTest extends PHPUnit_Framework_TestCase {
	...
	public function testSessionIsProperlySaved()
	{
		...
		$session->getHandler()->shouldReceive('write')->once()->with(
			$this->getSessionId(),
			m::on(function($data) use ($encrypter, $expected){
				return (serialize($expected) === $encrypter->decrypt($data));
			})
		);
		$session->save();
	...
	{
		return new Illuminate\Encryption\Encrypter('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
	}
