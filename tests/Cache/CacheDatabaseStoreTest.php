...
class CacheDatabaseStoreTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $store = $this->createMock('Illuminate\Cache\DatabaseStore', ['forget'], $this->getMocks());
        $table = m::mock('StdClass');
        $store->getConnection()->shouldReceive('table')->once()->with('table')->andReturn($table);
        $table->shouldReceive('where')->once()->with('key', '=', 'prefixfoo')->andReturn($table);
    ...
    {
        $store = $this->createMock('Illuminate\Cache\DatabaseStore', ['getTime'], $this->getMocks());
        $table = m::mock('StdClass');
        $store->getConnection()->shouldReceive('table')->once()->with('table')->andReturn($table);
    ...
    {
        $store = $this->createMock('Illuminate\Cache\DatabaseStore', ['put'], $this->getMocks());
