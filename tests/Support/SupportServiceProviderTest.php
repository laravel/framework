...
class SupportServiceProviderTest extends TestCase
{
    ...
    public function testRecursiveConfigMerging()
    {
        $app = m::mock('Illuminate\\Foundation\\Application')->makePartial();
        $provider = new ExtendedServiceProviderForTestingRecursiveConfigMerging($app);

        $array1 = ['foo'   => 'bar', 'names' => ['developer' => 'Taylor']];
        $array2 = ['foo'   => 'foo', 'names' => ['developer' => 'Abigail']];
        $this->assertEquals(['foo'   => 'foo', 'names' => ['developer' => 'Abigail']], $provider->testMergeConfigs($array1, $array2));

        $array1 = ['foo'   => 'bar', 'names' => ['Taylor']];
        $array2 = ['foo'   => 'foo', 'names' => ['Abigail']];
        $this->assertEquals(['foo'   => 'foo', 'names' => ['Taylor', 'Abigail']], $provider->testMergeConfigs($array1, $array2));

        $array1 = ['developers' => [['name' => 'Taylor']]];
        $array2 = ['developers' => [['name' => 'Abigail']]];
        ...
        $array2 = [0 => ['second' => 'Teacher']];
        $this->assertEquals([0  => ['first' => 'Programmer'], 1 => ['second' => 'Teacher']], $provider->testMergeConfigs($array1, $array2));

        $array1 = [0 => 'a string', 'first' => ['name' => 'Abigail']];
        $array2 = [0 => 420, 'first' => ['name'    => 'Taylor', 'surname' => 'Otwell']];
        $this->assertEquals([0 => 'a string', 1 => 420, 'first' => ['name'    => 'Taylor', 'surname' => 'Otwell']], $provider->testMergeConfigs($array1, $array2));
    }
}

class ExtendedServiceProviderForTestingRecursiveConfigMerging extends ServiceProvider
{
    protected $recursivelyMergeConfigs = true;

    public function testMergeConfigs($original, $merging)
    {
        return parent::mergeConfigs($original, $merging); // TODO: Change the autogenerated stub
    }
}
