...
class SupportArrTest extends TestCase
{
    ...
    public function testAdd()
    {
        ...
        $this->assertSame([1 => 'hAz'], Arr::add([], 1, 'hAz'));
        $this->assertSame([1 => [1 => 'hAz']], Arr::add([], 1.1, 'hAz'));

        // Case where the key already exists
        $this->assertSame(['type' => 'Table'], Arr::add(['type' => 'Table'], 'type', 'Chair'));
        $this->assertSame(['category' => ['type' => 'Table']], Arr::add(['category' => ['type' => 'Table']], 'category.type', 'Chair'));
    }
    ...
    public function testPush()
    {
        ...
        $this->assertSame(['Desk'], $array['office']['furniture']);

        Arr::push($array, 'office.furniture', 'Chair', 'Lamp');
        $this->assertSame(['Desk', 'Chair', 'Lamp'], $array['office']['furniture']);

        $array = [];

        Arr::push($array, null, 'Chris', 'Nuno');
        $this->assertSame(['Chris', 'Nuno'], $array);

        Arr::push($array, null, 'Taylor');
        $this->assertSame(['Chris', 'Nuno', 'Taylor'], $array);
    ...
    public function testCollapse()
    {
        ...
        $array = [[1], [2], [3], ['foo', 'bar']];
        $this->assertSame([1, 2, 3, 'foo', 'bar'], Arr::collapse($array));
        ...
        $this->assertSame([1, 2, 'foo', 'bar'], Arr::collapse($mixedArray));

        // Case including collections and arrays
        $collection = collect(['baz', 'boom']);
        $mixedArray = [[1], [2], [3], ['foo', 'bar'], $collection];
        $this->assertSame([1, 2, 3, 'foo', 'bar', 'baz', 'boom'], Arr::collapse($mixedArray));
    }
    ...
    public function testDivide(): void
    {
        ...
        $this->assertSame(['name', 'price', 'available'], $keys);
        $this->assertSame(['Desk', 100, true], $values);
    ...
    public function testUndot()
    {
        ...
        $this->assertSame(['user' => ['name' => 'Taylor', 'age' => 25, 'languages' => ['PHP', 'C#']]], $array);

        $array = Arr::undot([
            'pagination.previous' => '<<',
            'pagination.next' => '>>',
        ]);
        $this->assertSame(['pagination' => ['previous' => '<<', 'next' => '>>']], $array);
        ...
        $this->assertSame(['foo', 'foo' => ['bar' => 'baz', 'baz' => ['a' => 'b']]], $array);
    }
    ...
    public function testExcept()
    {
        ...
        $this->assertSame([1 => 'hAz'], Arr::except($array, 2));
        $this->assertSame([1 => 'hAz', 2 => [12 => 'baz']], Arr::except($array, 2.5));
    }

    public function testExceptValues()
    {
        $array = ['name' => 'taylor', 'age' => 26, 'city' => 'austin'];
        $this->assertSame(['name' => 'taylor', 'city' => 'austin'], Arr::exceptValues($array, [26]));
        $this->assertSame(['name' => 'taylor', 'city' => 'austin'], Arr::exceptValues($array, 26));

        $array = ['foo', 'bar', 'baz', 'qux'];
        $this->assertSame([1 => 'bar', 3 => 'qux'], Arr::exceptValues($array, ['foo', 'baz']));
        $this->assertSame([0 => 'foo', 1 => 'bar', 3 => 'qux'], Arr::exceptValues($array, 'baz'));

        $array = [1, 2, 3, 4, 5];
        $this->assertSame([0 => 1, 1 => 2, 4 => 5], Arr::exceptValues($array, [3, 4]));

        $array = ['a' => 1, 'b' => 2, 'c' => 1, 'd' => 3];
        $this->assertSame(['b' => 2, 'd' => 3], Arr::exceptValues($array, 1));

        $this->assertSame([], Arr::exceptValues([], 'foo'));
        $this->assertSame(['foo', 'bar'], Arr::exceptValues(['foo', 'bar'], []));

        $array = [1, '1', 2, '2', 3];
        $this->assertSame([1 => '1', 3 => '2'], Arr::exceptValues($array, [1, 2, 3], true));
        $this->assertSame([], Arr::exceptValues($array, [1, 2, 3]));

        $array = ['a' => true, 'b' => false, 'c' => 1, 'd' => 0];
        $this->assertSame(['a' => true, 'b' => false], Arr::exceptValues($array, [1, 0], true));
    ...
    public function testWhereNotNull(): void
    {
        ...
        $this->assertSame([1, 'string', 0.0, false, [], $class, $function], $array);
    }
    ...
    public function testOnly()
    {
        ...
        $this->assertSame(['foo'], Arr::only(['foo', 'bar', 'baz'], 0));
        $this->assertSame([1 => 'bar', 2 => 'baz'], Arr::only(['foo', 'bar', 'baz'], [1, 2]));
        $this->assertEmpty(Arr::only(['foo', 'bar', 'baz'], [3]));

        // Test with array having numeric key and string key
        $this->assertSame(['foo'], Arr::only(['foo', 'bar' => 'baz'], 0));
        $this->assertSame(['bar' => 'baz'], Arr::only(['foo', 'bar' => 'baz'], 'bar'));
    }

    public function testOnlyValues()
    {
        $array = ['name' => 'taylor', 'age' => 26, 'city' => 'austin'];
        $this->assertSame(['age' => 26], Arr::onlyValues($array, [26]));
        $this->assertSame(['age' => 26], Arr::onlyValues($array, 26));

        $array = ['foo', 'bar', 'baz', 'qux'];
        $this->assertSame([0 => 'foo', 2 => 'baz'], Arr::onlyValues($array, ['foo', 'baz']));
        $this->assertSame([2 => 'baz'], Arr::onlyValues($array, 'baz'));

        $array = [1, 2, 3, 4, 5];
        $this->assertSame([2 => 3, 3 => 4], Arr::onlyValues($array, [3, 4]));

        $array = ['a' => 1, 'b' => 2, 'c' => 1, 'd' => 3];
        $this->assertSame(['a' => 1, 'c' => 1], Arr::onlyValues($array, 1));

        $this->assertSame([], Arr::onlyValues([], 'foo'));
        $this->assertSame([], Arr::onlyValues(['foo', 'bar'], []));

        $array = [1, '1', 2, '2', 3];
        $this->assertSame([0 => 1, 2 => 2, 4 => 3], Arr::onlyValues($array, [1, 2, 3], true));
        $this->assertSame([0 => 1, 1 => '1', 2 => 2, 3 => '2', 4 => 3], Arr::onlyValues($array, [1, 2, 3]));

        $array = ['a' => true, 'b' => false, 'c' => 1, 'd' => 0];
        $this->assertSame(['c' => 1, 'd' => 0], Arr::onlyValues($array, [1, 0], true));
        $this->assertSame(['a' => true, 'b' => false, 'c' => 1, 'd' => 0], Arr::onlyValues($array, [1, 0]));
    }
    ...
    public function testMap()
    {
        ...
        $this->assertSame(['first' => 'first-rolyat', 'last' => 'last-llewto'], $mapped);
        $this->assertSame(['first' => 'taylor', 'last' => 'otwell'], $data);
    }
    ...
    public function testMapNullValues()
    {
        ...
        $this->assertSame(['first' => 'first-taylor', 'last' => 'last-'], $mapped);
    }
    ...
    public function testMapByReference()
    {
        ...
        $this->assertSame(['first' => 'rolyat', 'last' => 'llewto'], $mapped);
        $this->assertSame(['first' => 'taylor', 'last' => 'otwell'], $data);
    }
    ...
    public function testPrepend()
    {
        ...
        $this->assertSame(['' => 0, 'one' => 1, 'two' => 2], $array);

        $array = Arr::prepend(['one' => 1, 'two' => 2], 0, null);
        $this->assertSame([null => 0, 'one' => 1, 'two' => 2], $array);

        $array = Arr::prepend(['one', 'two'], null, '');
        $this->assertSame(['' => null, 'one', 'two'], $array);

        $array = Arr::prepend([], 'zero');
        $this->assertSame(['zero'], $array);

        $array = Arr::prepend([''], 'zero');
        $this->assertSame(['zero', ''], $array);

        $array = Arr::prepend(['one', 'two'], ['zero']);
        $this->assertSame([['zero'], 'one', 'two'], $array);

        $array = Arr::prepend(['one', 'two'], ['zero'], 'key');
        $this->assertSame(['key' => ['zero'], 'one', 'two'], $array);
    ...
    public function testSet()
    {
        ...
        $array = [1 => 'test'];
        $this->assertSame([1 => 'hAz'], Arr::set($array, 1, 'hAz'));
    }
    ...
    public function testShuffleKeepsSameValues()
    {
        ...
        $this->assertSame($input, $shuffled);
    }
    ...
    public function testForget()
    {
        ...
        $this->assertSame(['products' => ['desk' => [], '' => 'something']], $array);
        ...
        Arr::forget($array, 1);
        $this->assertSame(['name' => 'hAz', 2 => 'bAz'], $array);

        $array = [2 => [1 => 'products', 3 => 'users']];
        Arr::forget($array, 2.3);
        $this->assertSame([2 => [1 => 'products']], $array);
    }
    ...
    public function testTake(): void
    {
        ...
        $this->assertSame([1, 2, 3], Arr::take($array, 3));

        // Test with a negative limit, should return the last 'abs(limit)' elements.
        $this->assertSame([4, 5, 6], Arr::take($array, -3));
        ...
        $this->assertSame([1, 2, 3, 4, 5, 6], Arr::take($array, 10));
    ...
    public function testPartition()
    {
        ...
        $this->assertSame([[0 => 'John', 1 => 'Jane'], [2 => 'Greg']], $result);
    }
}
