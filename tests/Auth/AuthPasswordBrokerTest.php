...
class AuthPasswordBrokerTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $mocks = $this->getMocks();
        $broker = $this->createMock('Illuminate\Auth\Passwords\PasswordBroker', ['getUser', 'makeErrorRedirect'], array_values($mocks));
    ...
    {
        $mocks = $this->getMocks();
        $broker = $this->createMock('Illuminate\Auth\Passwords\PasswordBroker', ['emailResetLink', 'getUri'], array_values($mocks));
        $mocks['users']->shouldReceive('retrieveByCredentials')->once()->with(['foo'])->andReturn($user = m::mock('Illuminate\Contracts\Auth\CanResetPassword'));
    ...
    {
        $creds = ['token' => 'token'];
        $broker = $this->createMock('Illuminate\Auth\Passwords\PasswordBroker', ['validateNewPassword'], array_values($mocks = $this->getMocks()));
    ...
    {
        unset($_SERVER['__password.reset.test']);
        $broker = $this->createMock('Illuminate\Auth\Passwords\PasswordBroker', ['validateReset', 'getPassword', 'getToken'], array_values($mocks = $this->getMocks()));
