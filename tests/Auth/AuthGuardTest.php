...
class AuthGuardTest extends PHPUnit_Framework_TestCase {
	...
	public function testBasicReturnsNullOnValidAttempt()
	{
		...
		$guard = m::mock(Guard::class.'[check,attempt]', array($provider, $session));
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$guard = m::mock(Guard::class.'[check]', array($provider, $session));
	...
	{
		$guard = $this->getGuard();
		$guard->setDispatcher($events = m::mock(EventDispatcher::class));
	...
	public function testAttemptReturnsUserInterface()
	{
		...
		$guard->setDispatcher($events = m::mock(EventDispatcher::class));
		$events->shouldReceive('fire')->once()->with('auth.attempt', array(array('foo'), false, true));
		$user = $this->getMock(Authenticatable::class);
	...
	{
		$mock = $this->getGuard();
		$mock->setDispatcher($events = m::mock(EventDispatcher::class));
		$events->shouldReceive('fire')->once()->with('auth.attempt', array(array('foo'), false, true));
	...
	public function testLoginUsingIdStoresInSessionAndLogsInWithUser()
	{
		...
		$guard->getProvider()->shouldReceive('retrieveById')->once()->with(10)->andReturn($user = m::mock(Authenticatable::class));
	...
	protected function getCookieJar()
	{
		return new CookieJar(
			...
			array('domain' => 'foo.com', 'path' => '/', 'secure' => false, 'httpOnly' => false)
		);
	}
