    ...
    public function testPhpDeprecationsWithStackTraces()
    {
        ...
        $logger->shouldReceive('warning')->with(
            m::on(fn (string $message) => (bool) preg_match(
                "#ErrorException: str_contains\(\): Passing null to parameter \#2 \(\\\$needle\) of type string is deprecated in /home/user/laravel/routes/web\.php:17\s".
                "Stack trace:\s".
                "\#0 .*helpers.php\(.*\): Illuminate\\\\Foundation\\\\Bootstrap\\\\HandleExceptions.*\s".
                "\#1 .*HandleExceptions\.php\(.*\): with.*\s".
                "\#2 .*HandleExceptions\.php\(.*\): Illuminate\\\\Foundation\\\\Bootstrap\\\\HandleExceptions->handleDeprecation.*\s".
                "\#3 .*HandleExceptionsTest\.php\(.*\): Illuminate\\\\Foundation\\\\Bootstrap\\\\HandleExceptions->handleError.*\s".
                "[\s\S]*#i",
                $message
            ))
        );
