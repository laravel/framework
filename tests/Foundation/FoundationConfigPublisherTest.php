...
class FoundationConfigPublisherTest extends PHPUnit_Framework_TestCase {
	...
	public function testPackageConfigPublishing()
	{
		...
		$files2->shouldReceive('isDirectory')->once()->with(__DIR__.'/custom-packages/foo/bar/resources/config')->andReturn(true);
		$files2->shouldReceive('isDirectory')->once()->with(__DIR__.'/packages/foo/bar')->andReturn(true);
		$files2->shouldReceive('copyDirectory')->once()->with(__DIR__.'/custom-packages/foo/bar/resources/config', __DIR__.'/packages/foo/bar')->andReturn(true);
