...
class DatabaseMigrationRollbackCommandTest extends TestCase
{
    ...
    public function testRollbackCommandCallsMigratorWithProperArguments()
    {
        ...
        $migrator->shouldReceive('rollback')->once()->with([__DIR__.DIRECTORY_SEPARATOR.'migrations'], ['pretend' => false, 'step' => 0, 'batch' => 0, 'recursive' => false,]);
    ...
    public function testRollbackCommandCallsMigratorWithStepOption()
    {
        ...
        $migrator->shouldReceive('setOutput')->once()->andReturn($migrator);
        $migrator->shouldReceive('rollback')->once()->with([__DIR__.DIRECTORY_SEPARATOR.'migrations'], ['pretend' => false, 'step' => 2, 'batch' => 0, 'recursive' => false,]);
    ...
    public function testRollbackCommandCanBePretendedWithStepOption()
    {
        ...
        $migrator->shouldReceive('setOutput')->once()->andReturn($migrator);
        $migrator->shouldReceive('rollback')->once()->with([__DIR__.DIRECTORY_SEPARATOR.'migrations'], ['pretend' => true, 'step' => 2, 'batch' => 0, 'recursive' => false,]);
