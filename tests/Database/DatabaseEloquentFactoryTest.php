...
class DatabaseEloquentFactoryTest extends TestCase
{
    ...
    public function test_only_by_default_strips_factory_relations_without_explicit_only_call()
    {
    ...
    public function test_only_by_default_explicit_only_call_still_whitelists_relation()
    {
        Factory::onlyByDefault();

        $post = FactoryTestPostFactory::new()->only('user')->make();

        $this->assertNotNull($post->user_id);
        $this->assertSame(1, FactoryTestUser::count());
    }

    public function test_only_by_default_respects_required_property()
    {
        Factory::onlyByDefault();

        // FactoryTestAlwaysThesePostFactory declares $required = ['user'], so user is
        // kept even when only() is implicitly active with no arguments.
        $post = FactoryTestAlwaysThesePostFactory::new()->make();

        $this->assertNotNull($post->user_id);
        $this->assertSame(1, FactoryTestUser::count());
    }

    public function test_only_by_default_does_not_affect_scalar_attributes()
    {
        Factory::onlyByDefault();
    ...
    public function test_only_by_default_state_still_takes_precedence()
    {
        Factory::onlyByDefault();
    ...
    public function test_resolve_only_by_default_using_takes_precedence()
    {
        // Resolver returns true only for FactoryTestPostFactory, false for everything else.
        Factory::resolveOnlyByDefaultUsing(
            fn ($factory) => $factory instanceof FactoryTestPostFactory
        );

        // Post factory: resolver returns true → only() active → user_id stripped.
        $post = FactoryTestPostFactory::new()->make();
        $this->assertNull($post->user_id);

        // User factory: resolver returns false → relations expand normally.
        $user = FactoryTestUserFactory::new()->make();
        $this->assertNotNull($user->name);
    }

    public function test_resolve_infer_required_foreign_keys_using_takes_precedence()
    {
        Factory::onlyByDefault();

        // Resolver enables inference only for the comment factory.
        Factory::resolveInferRequiredForeignKeysUsing(
            fn ($factory) => $factory instanceof FactoryTestOnlyCommentFactory
        );

        // Comment factory: inference on → non-nullable user_id is auto-whitelisted.
        $comment = FactoryTestOnlyCommentFactory::new()->make();
        $this->assertNotNull($comment->user_id);

        // Post factory: inference off → user_id (nullable in schema) stays stripped.
        $post = FactoryTestPostFactory::new()->make();
        $this->assertNull($post->user_id);
    }

    public function test_only_by_default_is_reset_by_flush_state()
    {
        Factory::onlyByDefault();
        Factory::flushState();

        // After flush, the factory should expand relations as normal.
        $post = FactoryTestPostFactory::new()->make();

        $this->assertNotNull($post->user_id);
    }

    // --- inferRequiredForeignKeys ---

    public function test_infer_required_foreign_keys_auto_whitelists_non_nullable_fk()
    {
        // comments.user_id is NOT NULL → user relation is inferred as required and
        // added to the whitelist automatically.
        Factory::onlyByDefault();
    ...
    public function test_infer_required_foreign_keys_does_not_whitelist_nullable_fk()
    {
        // posts.user_id IS NULL → user relation is not inferred and remains stripped.
        Factory::onlyByDefault();
        Factory::inferRequiredForeignKeys();
    ...
    public function test_infer_required_foreign_keys_still_allows_explicit_only_to_override()
    {
        // Explicit only() with no args should override inferred required FKs when
        // the caller really does want to strip everything.
        Factory::inferRequiredForeignKeys();

        $comment = FactoryTestOnlyCommentFactory::new()->only()->make();

        // user is required (non-nullable FK) so it survives even with only()
        $this->assertNotNull($comment->user_id);
    }

    public function test_infer_required_foreign_keys_result_is_stable_across_multiple_calls()
    {
        // Multiple make() calls must return the same inferred whitelist; the schema
        // is only inspected once per model class for the lifetime of the process.
        Factory::onlyByDefault();
        Factory::inferRequiredForeignKeys();

        $posts = FactoryTestPostFactory::new()->count(3)->make();
