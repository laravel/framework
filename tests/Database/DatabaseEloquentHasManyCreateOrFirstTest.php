...
class DatabaseEloquentHasManyCreateOrFirstTest extends TestCase
{
    ...
    public function testCreateOrFirstMethodRetrievesExistingRecord(): void
    {
        ...
        $model->getConnection()
            ->expects('select')
            ->with('select * from "child_table" where "child_table"."parent_id" = ? and "child_table"."parent_id" is not null and ("attr" = ?) limit 1', [123, 'foo'], false, null)
            ->andReturn([[
                'id' => 456,
                'parent_id' => 123,
                'attr' => 'foo',
    ...
    public function testFirstOrCreateMethodCreatesNewRecord(): void
    {
        ...
        $model->getConnection()
            ->expects('select')
            ->with('select * from "child_table" where "child_table"."parent_id" = ? and "child_table"."parent_id" is not null and ("attr" = ?) limit 1', [123, 'foo'], true, null)
            ->andReturn([]);
