...
class DatabaseEloquentBuilderTest extends TestCase
{
    ...
    public function testRelationshipEagerLoadProcessForImplicitlyEmptys()
    {
        $queryBuilder = $this->getMockQueryBuilder();
        $builder = m::mock(Builder::class.'[getRelation]', [$queryBuilder]);
        $builder->setEagerLoads(['qux' => function ($query) {
            $_SERVER['__eloquent.constrain'] = $query;
        }]);
        $model = new EloquentBuilderTestModelSelfRelatedStub;
        $this->mockConnectionForModel($model, 'SQLite');
        ...
        $relation = m::mock($model->qux());

        $builder->shouldReceive('getRelation')->once()->with('qux')->andReturn($relation);
