...
class DatabaseConnectorTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $config = ['host' => 'foo', 'database' => 'bar', 'port' => 111, 'appname' => 'baz', 'readonly' => true, 'charset' => 'utf-8', 'connectionPooling' => false];
        $dsn = $this->getDsn($config);
        $connector = $this->getMock('Illuminate\Database\Connectors\SqlServerConnector', ['createConnection', 'getOptions']);
        $connection = m::mock('stdClass');
        $connector->expects($this->once())->method('getOptions')->with($this->equalTo($config))->will($this->returnValue(['options']));
    ...
    protected function getDsn(array $config)
    {
        ...
        if (in_array('dblib', PDO::getAvailableDrivers())) {
            ...
            $connectionPooling = (isset($config['connectionPooling']) && $config['connectionPooling'] == false) ? ';ConnectionPooling=0' : '';

            return "sqlsrv:Server={$host}{$port};Database={$database}{$appname}{$readonly}{$connectionPooling}";
        }
    }
}
