...
class DatabaseSqlServerSchemaGrammarTest extends TestCase
{
    ...
    public function testDropColumnDropsCreatesSqlToDropDefaultConstraints()
    {
        ...
        $this->assertCount(1, $statements);
        $this->assertEquals("DECLARE @sql NVARCHAR(MAX) = '';SELECT @sql += 'ALTER TABLE [dbo].[foo] DROP CONSTRAINT ' + OBJECT_NAME([default_object_id]) + ';' FROM MASTER.SYS.COLUMNS WHERE [object_id] = OBJECT_ID('[dbo].[foo]') AND [name] in ('bar') AND [default_object_id] <> 0;EXEC(@sql);alter table \"foo\" drop column \"bar\"", $statements[0]);
    }
