...
class RoutingUrlGeneratorTest extends TestCase
{
    ...
    public function testBasicRouteGeneration()
    {
        ...
        $this->assertEquals('http://www.foo.com/foo/bar', $url->route('optional'));
        $this->assertEquals('http://www.foo.com/foo/bar/taylor', $url->route('optional', 'taylor'));
        $this->assertEquals('http://www.foo.com/foo/bar/taylor', $url->route('optional', ['taylor']));
        $this->assertEquals('http://www.foo.com/foo/bar/taylor?breeze', $url->route('optional', ['taylor', 'breeze']));
        $this->assertEquals('http://www.foo.com/foo/bar/taylor?wall=woz', $url->route('optional', ['wall' => 'woz', 'taylor']));
        $this->assertEquals('http://www.foo.com/foo/bar/taylor?wall=woz&breeze', $url->route('optional', ['wall' => 'woz', 'breeze', 'baz' => 'taylor']));
        $this->assertEquals('http://www.foo.com/foo/bar?wall=woz', $url->route('optional', ['wall' => 'woz']));
