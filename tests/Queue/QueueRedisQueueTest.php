...
class QueueRedisQueueTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $queue = $this->createMock('Illuminate\Queue\RedisQueue', ['getRandomId'], [$redis = m::mock('Illuminate\Redis\Database'), 'default']);
    ...
    {
        $queue = $this->createMock('Illuminate\Queue\RedisQueue', ['getSeconds', 'getTime', 'getRandomId'], [$redis = m::mock('Illuminate\Redis\Database'), 'default']);
        $queue->expects($this->once())->method('getRandomId')->will($this->returnValue('foo'));
    ...
    {
        $queue = $this->createMock('Illuminate\Queue\RedisQueue', ['getTime', 'migrateAllExpiredJobs'], [$redis = m::mock('Illuminate\Redis\Database'), 'default']);
    ...
    {
        $queue = $this->createMock('Illuminate\Queue\RedisQueue', ['getTime'], [$redis = m::mock('Illuminate\Redis\Database'), 'default']);
        $queue->expects($this->once())->method('getTime')->will($this->returnValue(1));
        $redis->shouldReceive('connection')->once()->andReturn($redis);
    ...
    {
        $queue = $this->createMock('Illuminate\Queue\RedisQueue', ['getTime', 'migrateAllExpiredJobs'], [$redis = m::mock('Illuminate\Redis\Database'), 'default', null]);
