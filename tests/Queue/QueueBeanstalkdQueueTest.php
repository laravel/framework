...
class QueueBeanstalkdQueueTest extends TestCase
{
    ...
    public function testPushProperlyPushesJobOntoBeanstalkd()
    {
        ...
        $pheanstalk->shouldReceive('put')->twice()->with(json_encode(['uuid' => $uuid, 'displayName' => 'foo', 'job' => 'foo', 'maxTries' => null, 'maxExceptions' => null, 'failOnTimeouts' => false, 'backoff' => null, 'timeout' => null, 'data' => ['data']]), 1024, 0, 60);
    ...
    public function testDelayedPushProperlyPushesJobOntoBeanstalkd()
    {
        ...
        $pheanstalk->shouldReceive('useTube')->once()->with('stack')->andReturn($pheanstalk);
        $pheanstalk->shouldReceive('useTube')->once()->with('default')->andReturn($pheanstalk);
        $pheanstalk->shouldReceive('put')->twice()->with(json_encode(['uuid' => $uuid, 'displayName' => 'foo', 'job' => 'foo', 'maxTries' => null, 'maxExceptions' => null, 'failOnTimeouts' => false, 'backoff' => null, 'timeout' => null, 'data' => ['data']]), Pheanstalk::DEFAULT_PRIORITY, 5, Pheanstalk::DEFAULT_TTR);
