...
class ValidationValidatorTest extends PHPUnit_Framework_TestCase {
	...
	public function testValidateUnique()
	{
		...
		$v = new Validator($trans, array('email' => 'foo'), array('email' => 'Unique:users,email_addr,1,id_col,col,2'));
		$mock3 = m::mock('Illuminate\Validation\PresenceVerifierInterface');
		$mock3->shouldReceive('getCount')->once()->with('users', 'email_addr', 'foo', '1', 'id_col', array('col' => 2))->andReturn(2);
		$v->setPresenceVerifier($mock3);
		$this->assertFalse($v->passes());
	}
	...
	public function testValidationExists()
	{
		...
		$v = new Validator($trans, array('email' => array('foo')), array('email' => 'Exists:users,email_addr,col,2'));
		$mock3 = m::mock('Illuminate\Validation\PresenceVerifierInterface');
		$mock3->shouldReceive('getMultiCount')->once()->with('users', 'email_addr', array('foo'), array('col' => 2))->andReturn(false);
		$v->setPresenceVerifier($mock3);
		$this->assertFalse($v->passes());
	}
