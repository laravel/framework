...
class Str {
   ...
   */
	...
	/**
	 * List of words to be removed on slug call..
	 *
	 * @var array
	 */
	...
	public function ascii($value)
	{
		...
		if (isset($this->ascii[$this->language]))
		{
			foreach ($this->ascii[$this->language] as $foreign => $char)
			{
				$map["/$foreign/"] = $char;
			}
		}
	...
	public function slug($title, $separator = '-')
	{
		...
		// Remove certain words (conjunctions, fillers) before transliterating
		$title = preg_replace ('/\b(' . join ('|', $this->removeList) . ')\b/i', '', $title);
	...
	/**
	 * Set additional transliteration map.
	 *
	 * @param  array  $map
	 * @return void
	 */
	public function setAsciiMap($map)
	{
		foreach ($map as $key => $chars)
		{
			if (isset($this->ascii[$key]))
			{
				$this->ascii[$key] = array_merge($this->ascii[$key], $chars);
			}
			else
			{
				$this->ascii[$key] = $chars;
			}
		}
	}

	/**
	 * Set additional words to remove on slug method.
	 *
	 * @param  array  $words
	 * @return void
	 */
	public function setRemoveList($words)
	{
		$this->removeList = array_merge($this->removeList, (array) $words);
	}

	/**
	 * Set priority language.
	 *
