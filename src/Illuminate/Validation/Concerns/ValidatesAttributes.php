...
trait ValidatesAttributes
{
    ...
    /**
     * Validate that an attribute is url and it's protocol are one of the available protocols.
     *
     * @param  string  $attribute
     * @param  mixed  $value
     * @param  mixed  $parameters
     * @return bool
     */
    public function validateUrlProtocol($attribute, $value, $parameters)
    {
        $availableProtocols = [
            "aaa",
            "aaas",
        ...
        foreach ($parameters as $protocl) {
            if (!in_array($protocl, $availableProtocols)) {
                throw new InvalidArgumentException("Not supported protocol provided.");
            }
        }

        $parsedUrl = parse_url($value);
        return filter_var($value, FILTER_VALIDATE_URL)
            && empty($parsedUrl['scheme']) === false
            && in_array($parsedUrl['scheme'], $parameters);
    }
}
