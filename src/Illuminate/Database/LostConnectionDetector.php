...
class LostConnectionDetector implements LostConnectionDetectorContract
{
    ...
    {
        if ($this->causedByLostConnectionByErrorInfo($e)) {
            return true;
        }
    ...
    /**
     * Determine if the given exception was caused by a lost connection based on locale-independent error info.
     *
     * @param  \Throwable  $e
     * @return bool
     */
    protected function causedByLostConnectionByErrorInfo(Throwable $e): bool
    {
        if (! $e instanceof PDOException || ! isset($e->errorInfo[0], $e->errorInfo[1])) {
            return false;
        }

        return (string) $e->errorInfo[0] === 'HY000'
