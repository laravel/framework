...
class Batch implements Arrayable, JsonSerializable
{
    ...
    public function recordSuccessfulJob(string $jobId)
    {
        ...
        if ($this->hasProgressCallbacks()) {
            ...
            (new Collection($this->options['progress']))
                ->each(fn ($handler) => $this->invokeHandlerCallback($handler, $batch));
        }
        ...
        if ($counts->pendingJobs === 0 && $this->hasThenCallbacks()) {
            ...
            (new Collection($this->options['then']))
                ->each(fn ($handler) => $this->invokeHandlerCallback($handler, $batch));
        }
        ...
        if ($counts->allJobsHaveRanExactlyOnce() && $this->hasFinallyCallbacks()) {
            ...
            (new Collection($this->options['finally']))
                ->each(fn ($handler) => $this->invokeHandlerCallback($handler, $batch));
        }
    }
    ...
    public function recordFailedJob(string $jobId, $e)
    {
        ...
        if ($this->hasProgressCallbacks() && $this->allowsFailures()) {
            ...
            (new Collection($this->options['progress']))
                ->each(fn ($handler) => $this->invokeHandlerCallback($handler, $batch, $e));
        }

        if ($counts->failedJobs === 1 && $this->hasCatchCallbacks()) {
            $batch = $this->fresh();

            (new Collection($this->options['catch']))
                ->each(fn ($handler) => $this->invokeHandlerCallback($handler, $batch, $e));
        }
