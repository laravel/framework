...
class QueueServiceProvider extends ServiceProvider {
	 ...
	 * @return void
	 */
	protected function registerRequestBinder($queue)
	{
		$this->app->rebinding('request', function($app, $request) use ($queue)
		{
			if ($app['queue']->connected($queue))
			{
				$app['queue']->connection($queue)->setRequest($request);
			}
		});
	}
