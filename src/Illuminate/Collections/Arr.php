...
class Arr
{
    ...
    /**
     * Scales the values of an array to a specified range, using minimum and maximum values from the array
     *
     * @param array $array
     * @param int $min_range
     * @param int $max_range
    ...
    public static function scale($array, $min_range = 0, $max_range = 1,$precision=14)
    {
        ...
        return self::map($array, function ($value) use ($min_range, $max_range, $arr_min, $arr_max,$precision) {
            return round($min_range + (($value - $arr_min) / ($arr_max - $arr_min)) * ($max_range - $min_range),$precision);
        });
    }

    /**
     * Scales the selected array values to a specified range
     *
     * @param array $array
     * @param array|string $keys
     * @param int $min_range
     ...
     * @return array
     */
    public static function scaleBy($array, $keys, $min_range = 0, $max_range = 1,$precision=14)
    {
        if (empty($array)) {
            return [];
        }
    ...
    /**
     * Scales the selected array indexes values to a specified range
     *
     * @param array|string $keys
     * @param  int  $min_range
     * @param  int  $max_range
     ...
     * @return array
     */
    public static function scaleByMany($array, $keys, $min_range = 0, $max_range = 1,$precision=14)
    {
        if (empty($array)) {
            return [];
        }
        ...
        foreach ($keys as $key => $value) {
            ...
            if(collect($values)->unique()->toArray()==[null])
                continue;
            ...
            $array = self::map($array, function ($item, $k) use ($key, $result) {
