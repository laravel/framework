                ...
                @else
                    <div id="CopyQuickEstimateApplication">
                        <form id="CopyQuickEstimateForm" method="POST" action="{{ $storeRoute }}" data-bootstrap-url="{{ $bootstrapRoute }}">
                            @include("quick-estimates.customer.partials.copy.box-header")
                            <div class="box-body pd-0 table-responsive">
                                @include("quick-estimates.customer.partials.copy.sticky-header")
                                <div id="CopyQuickEstimate">
                                    @include("quick-estimates.customer.partials.copy.table")
                                </div>
                            ...
                            @include("quick-estimates.customer.partials.copy.notes")
                        ...
                        {{-- Update rooms modal --}}
                        @include("quick-estimates.customer.partials.copy.rooms")
                        {{-- Specifications and ratecards modals --}}
                        @include("quick-estimates.customer.partials.copy.specifications")
                        @include("quick-estimates.customer.partials.copy.ratecards")
                    </div>
                @endif
            </div>
        </div>
...
@section("dynamicScripts")
    ...
    <script src="{{ asset("js/quick-estimates/copy.min.js") }}"></script>
@endsection
