        ...
        <tbody>
            <tr v-for="(laminate, index) in suggestionsList">
                ...
                <td class="text-vertical-align text-center pd-0" width="16%">
                    <span title="Add" class="cursor-pointer" @click.prevent="addLamToCombination(laminate.LaminateId, laminate)" :id="laminate.Id" v-if="!laminate.Active">
        ...
        methods: {
            ...
            addLamToCombination(id, laminate) {
                this.$emit("selectlaminate", {
                    "Id": id,
                    "Laminate": laminate,
                    "Combinations": this.$root.SearchResult
                });
            },
            getBrand($brandId) {
