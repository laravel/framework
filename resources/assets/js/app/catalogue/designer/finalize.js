...
const VueInstance = new Vue({
    ...
    data: {
        ...
        RoomId: null, // Stores Combination's Room Id
        selectedRoomArea: null, //Stores Combination Room Name
...
//        this.Laminates = this.addProperty(Laminates); // Assign existing laminates to vue variable
        ...
        this.Status = this.SelectedCombLaminates[0].Status;
    ...
    methods: {
        ...
        // Yes detach other selections with same room
        ...
        InitializeValidator() {
            ValidatorObj = $("#FinalizeCombination").validate({
                ...
                messages: {
                    ...
                    Action: {
                        required: "Please select a Action."
                    },
                    Rooms: {
...
$(document).ready(function () {
    ...
    $("#FinalizeCombSubmitBtn").on('click', function (event) {
        event.preventDefault();
//        VueInstance.RoomId = jquery("#Rooms").val();
        // User has to select at least one laminate from filters
        if (VueInstance.SelectedCombLaminates.length < 1) {
            VueInstance.populateNotifications({
                status: "warning",
                message: "Please select at least one laminate from Pick from Shortlist or Search section."
            });
            return;
        }
        if (jquery("#Rooms").val() && jquery("input[name='Action']:checked").val() == 4) {
