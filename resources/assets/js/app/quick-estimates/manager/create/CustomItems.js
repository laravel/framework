...
class CustomItems
{
    ...
    execute(createFormSelector, updateFormSelector)
    {
        ...
        this.initializeCustomItemUnitSelect2();
    ...
    registerUpdateCustomItemModalEvents()
    {
        ...
        $("#UpdateCustomItemModal").on("shown.bs.modal", () => {
            $("#UpdateCustomItemRoom, #UpdateCustomItemUnit, #UpdateCustomItemCategory").trigger("change");
    ...
    /**
     * Initialize create and update custom item unit select2.
     *
     * @return void
     */
    initializeCustomItemUnitSelect2()
    {
        $("select.custom-item-unit").each(function () {
            let node = $(this);
            node.select2({
                "placeholder": "Select a unit from dropdown",
            }).on("select2:select", () => {
                return node.val().length !== 0 && node.valid();
            });
            applyKeyboardNavigationPatch(node);
