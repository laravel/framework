...
class CopyEstimate extends Application
{
    ...
    /**
     * Initialize estimate application, form requests.
     *
     * @return void
     */
    initializeApplication()
    {
        ...
        let copyFormRequests = new FormRequests(this.notifier, this.translator, this.formRequisites, this.application);
        this.formRequests = copyFormRequests.execute();
        ...
        $("#CopyQuickEstimateForm").on("qe.items.ready-to-submit", (event, form, formData) => {
    ...
    registerGotoTop()
    {
        $("#CopyQuickEstimateGoToTop").on("click", () => $(window).scrollTop(0));
    }
    ...
    {
        $("#CopyQuickEstimateTable").on("qe.items.ready", () => this.initializeItemsWidgets());
    }
    ...
    {
        $("#CopyQuickEstimateTable").on("qe.items.not-selected", () => {
            let estimationTableNode = $("form#CopyQuickEstimateForm > div.table-responsive");
    ...
    {
        $("#CopyQuickEstimateTable").on("qe.items.widgets.destroy", () => {
    ...
    {
        let estimationStickyHeaderNode = $("#CopyQuickEstimateStickyHeader"),
            estimationTableNode = $("#CopyQuickEstimateTable");
...
$(document).ready(() => (new CopyEstimate()).execute());
