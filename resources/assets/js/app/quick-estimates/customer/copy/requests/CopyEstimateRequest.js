...
/**
 * CopyEstimateRequest class for validating copy estimate form.
 *
 * Defines rules and messages for the CopyEstimateForm validation.
 * Adds sentence validation method and handles ajax form submission.
 */
class CopyEstimateRequest extends Request
{
    /**
     ...
     * @var Selector
     */
    formSelector = "#CopyQuickEstimateForm";
    ...
    /**
     * Get notifier for the CopyEstimateRequest.
     *
    ...
    /**
     * Get translator for the CopyEstimateRequest.
     *
    ...
    /**
     * Get form requisites for the CopyEstimateRequest.
     *
    ...
    /**
     * Initialize essentials for the CopyEstimateRequest.
     *
     ...
     * @param  Element  form
     * @param  Event  event
     * @param  ./CopyEstimateRequest  that
     * @return void
     */
    handler(form, event, that)
    {
        ...
        $(window).scrollTop($("form#CopyQuickEstimateForm > div.table-responsive").height() / 2);
        ...
        if (that.application.customItems.length > 0) {
            ...
            $("#CopyQuickEstimateForm").trigger("qe.items.ready-to-submit", [form, formData]);
        }
    }
    ...
    decodeImagesAndSubmit(form, formData)
    {
        ...
        _.forEach(this.application.customItems, (customItem) => {
            if (_.isPlainObject(customItem.image) && ! _.has(customItem.image, "url")) {
...
export default CopyEstimateRequest;
